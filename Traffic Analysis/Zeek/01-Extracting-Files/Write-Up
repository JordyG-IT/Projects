...
![Identifying File types](Picture3.png)

### 5. We can use hashes to check if the files are malicious using sites like Virus total. We find the Hashes by running Zeek's Hash framework.
zeek -C -r case.pcap hash-demo.zeek

![Running the Hash Framework](Picture4.png)

### 6. We can now look at the files.log to grab the md5 sha1 or sha256 hash of the suspicious executable

cat files.log | zeek-cut conn_uids md5 | grep"CIt2r51C0rT9eOlaVl"

![Grabbing the hash to check against CTI](Picture5.png)
